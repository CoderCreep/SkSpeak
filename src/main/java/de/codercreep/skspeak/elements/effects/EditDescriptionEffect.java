...
public class EditDescriptionEffect extends Effect {
    ...
    public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean kleenean, SkriptParser.ParseResult paramParseResult) {
        ...
        this.description = (Expression<String>) expressions[1];
        return true;
    }
    ...
    protected void execute(Event event) {
        ...
        SkSpeak.getInstance().getTs3Api().editChannel(this.id.getSingle(event), ChannelProperty.CHANNEL_DESCRIPTION, this.description.getSingle(event));
    }
}
